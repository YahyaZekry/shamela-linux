...
def searchAllowed(info):
    ...
    if not not_panels:
        return True
    if not requiresMemoryIndex(info):
        return True
...
class SearchWidget(QWidget):
    ...
    def restoreFocusState(self, widget, state):
        if not (state and hasattr(widget, 'setCursorPosition')):
            return
    ...
    def restoreRememberedFocus(self):
        ...
        self._pending_focus_state = None
        try:
            if not (widget and self.ownsWidget(widget)):
                return
            if widget.isHidden() or widget.isEnabled() and widget.focusPolicy() == Qt.NoFocus:
                return
            widget.setFocus()
                ...
                if phrase.isdigit():
                    ...
                    if self.stemCheck.isChecked():
                        if '*' in phrase:
                            ...
                            return
                    if ' * ' in phrase or phrase.startswith('* ') or phrase.endswith(' *'):
            ...
            for panel_dict in panels_list:
                if panel_dict['phrases']:
                    if panel_dict['type'] == 0:
                        ...
                        if panel_dict['type'] == 1:
                            or_lists.append(panel_dict['phrases'])
                    if panel_dict['type'] == 2:
    ...
    def ensureOne(self, i, checked):
        ...
        if checked:
            if i == 3:
                ...
                self.options[3].setChecked(False)
        else:
            anyone = False
            for i in range(4):
