...
def detect_running_arch():
    return normalize_arch(launched_arch()) or ('32' if sys.maxsize <= 4294967296 else '64')
...
def detect_system_arch():
    ...
    if not Across.os == 'mac' or arch == '64':
        try:
...
def top_ver_info(folder):
    ...
    for subfolder in subfolders:
        if subfolder.isdigit():
            if os.path.isfile(os.path.join(folder, subfolder, 'done.manifest')):
    ...
    return (
     os.path.join(folder, f"{version}"), version)
...
def _install_win_launcher_from_zip(zf):
    try:
        ...
        if not (os.path.isfile(target_path) and _file_crc32(target_path) == entry.CRC):
...
if __name__ == '__main__':
    ...
    if Across.os == 'win':
        ...
        if not (jre_path and lucene_path):
            show_error_dialog(REDOWNLOAD)
            sys.exit()
