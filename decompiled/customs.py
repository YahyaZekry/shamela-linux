...
def _finalizeClipboardOnExit():
    ...
    if not state.get('active'):
        ...
        return _ensureNativeClipboardApi() or None
    try:
        pasteboard = NSPasteboard.generalPasteboard()
...
def _writeFileurlPasteboard(xml_path):
    ...
    return _ensureNativeClipboardApi() and NSURL or None
    pasteboard = NSPasteboard.generalPasteboard()
    if pasteboard is None:
        return
    ...
    for chunk in style_text.split(';'):
        ...
        if key == 'font-size':
            ...
            if size:
                ...
                style['bold'] = lowered in frozenset({'900', 'bold', '700', '600', '800'})
            elif key == 'font-style':
        ...
        if tag == 'font':
            ...
            if size:
                ...
                if tag in frozenset({'b', 'strong'}):
                    inherited['bold'] = True
            elif tag in frozenset({'i', 'em'}):
                inherited['italic'] = True
            else:
                if tag in frozenset({'p', 'span', 'body', 'font'}):
                    pass
        if tag in frozenset({'span', 'strong', 'p', 'em', 'font', 'b', 'i', 'body'}):
...
def _attributionFont():
    ...
    return [fp[0], Settings.getValue('attribute_size'), fp[2], fp[3]]
...
def enrichMimeData(data):
    ...
    if Across.os != 'mac':
        ...
        return data is None or data.hasHtml() or data
        return data.data('text/rtf').isEmpty() and data.data('application/rtf').isEmpty() or data
...
class BlockSize:
    ...
    def getBlockSize(cls):
        drive_name = cls._size or os.path.splitdrive(Across.bin_directory)[0]
...
def realResolutions_new--- This code section failed: ---

 L.1016         0  LOAD_GLOBAL              Across
                2  LOAD_ATTR                os
                4  LOAD_STR                 'win'
                6  COMPARE_OP               !=
                8  POP_JUMP_IF_TRUE     18  'to 18'
               10  LOAD_GLOBAL              win32api
               12  POP_JUMP_IF_FALSE    18  'to 18'
               14  LOAD_GLOBAL              ctypes
               16  POP_JUMP_IF_TRUE     22  'to 22'
             18_0  COME_FROM            12  '12'
             18_1  COME_FROM             8  '8'

 L.1017        18  LOAD_CONST               None
               20  RETURN_VALUE     
             22_0  COME_FROM            16  '16'

 L.1019        22  SETUP_EXCEPT        166  'to 166'

 L.1020        24  LOAD_CONST               0
               26  STORE_FAST               'MDT_EFFECTIVE_DPI'

 L.1021        28  LOAD_GLOBAL              ctypes
               30  LOAD_ATTR                windll
               32  LOAD_ATTR                shcore
               34  STORE_FAST               'shcore'

 L.1022        36  LOAD_FAST                'shcore'
               38  LOAD_ATTR                GetDpiForMonitor
               40  STORE_FAST               'get_dpi'

 L.1023        42  LOAD_GLOBAL              win32api
               44  LOAD_METHOD              EnumDisplayMonitors
               46  CALL_METHOD_0         0  '0 positional arguments'
               48  STORE_FAST               'monitors'

 L.1024        50  BUILD_LIST_0          0 
               52  STORE_FAST               'resolutions'

 L.1025        54  SETUP_LOOP          158  'to 158'
               56  LOAD_FAST                'monitors'
               58  GET_ITER         
               60  FOR_ITER            156  'to 156'
               62  STORE_FAST               'monitor'

 L.1026        64  LOAD_GLOBAL              ctypes
               66  LOAD_METHOD              c_uint
               68  CALL_METHOD_0         0  '0 positional arguments'
               70  STORE_FAST               'dpiX'

 L.1027        72  LOAD_GLOBAL              ctypes
               74  LOAD_METHOD              c_uint
               76  CALL_METHOD_0         0  '0 positional arguments'
               78  STORE_FAST               'dpiY'

 L.1028        80  LOAD_FAST                'get_dpi'

 L.1029        82  LOAD_FAST                'monitor'
               84  LOAD_CONST               0
               86  BINARY_SUBSCR    
               88  LOAD_ATTR                handle

 L.1030        90  LOAD_FAST                'MDT_EFFECTIVE_DPI'

 L.1031        92  LOAD_GLOBAL              ctypes
               94  LOAD_METHOD              byref
               96  LOAD_FAST                'dpiX'
               98  CALL_METHOD_1         1  '1 positional argument'

 L.1032       100  LOAD_GLOBAL              ctypes
              102  LOAD_METHOD              byref
              104  LOAD_FAST                'dpiY'
              106  CALL_METHOD_1         1  '1 positional argument'
              108  CALL_FUNCTION_4       4  '4 positional arguments'
              110  STORE_FAST               'hresult'

 L.1034       112  LOAD_FAST                'hresult'
              114  LOAD_CONST               0
              116  COMPARE_OP               !=
 ...
 L.1035       132  LOAD_CONST               None
              134  RETURN_VALUE     
            136_0  COME_FROM           130  '130'

 L.1036       136  LOAD_FAST                'resolutions'
              ...
              152  POP_TOP          
              154  JUMP_BACK            60  'to 60'
              156  POP_BLOCK        
            158_0  COME_FROM_LOOP       54  '54'

 L.1037       158  LOAD_FAST                'resolutions'
              160  JUMP_IF_TRUE_OR_POP   164  'to 164'
              162  LOAD_CONST               None
            164_0  COME_FROM           160  '160'
              164  RETURN_VALUE     
            166_0  COME_FROM_EXCEPT     22  '22'
...
def realResolutions_old():
    ...
    """
    return Across.os != 'win' or ctypes or None
    try:
        LOGPIXELSX = 88
        LOGPIXELSY = 90
...
def _macLiteralControlShortcut(sequence):
    ...
    if (Across.os != 'mac' or 'Ctrl') not in portable:
        if 'Control' not in portable:
            return portable
...
def _shortcutVariants(sequence, wrapped=True):
    ...
    if not wrapped:
        return [
         _macLiteralControlShortcut(portable)]
    variants = [portable]
...
def QtFont(l_font, scale=True):
    ...
    if l > 2:
        if l > 4:
            isinstance(l_font[4], str) or font.setWeight(fontSettingWeight(l_font))
        else:
...
def fontSettingWeight(l_font):
    if len(l_font) > 4:
        if not isinstance(l_font[4], str):
            try:
                return int(l_font[4])
...
def isZipValid(file):
    if not os.path.isfile(file):
        ...
        return zipfile.is_zipfile(file) or None
    return True
    ...
    def keyPressEvent(self, event):
        ...
        if modifiers & Qt.ControlModifier:
            ...
            if not Qt.Key_F1 <= key <= Qt.Key_F35:
                if key in {Qt.Key_Left, Qt.Key_Right, Qt.Key_PageUp, Qt.Key_PageDown, Qt.Key_End, Qt.Key_Copy, Qt.Key_End, Qt.Key_Home}:
                    super().keyPressEvent(event)
...
class ServiceBooks(QWidget):
    ...
    def displayItem(self):
        ...
        elif not self.book_list.selectedItems():
            return
            ...
            for book in self.books:
                if self.filtred(book[0]):
                    ...
                    if not book[2]:
                        listItem.setFlags(Qt.NoItemFlags)
                    self.book_list.addItem(listItem)
                    if no_selection and self.lastbook and self.lastbook == book[0] and book[2]:
                        no_selection = None
    ...
    def saveEditor(self):
        ...
        for i in range(count):
            ...
            if book_id and item.flags() & Qt.ItemIsUserCheckable:
