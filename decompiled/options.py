...
def lFont(qt_font):
    ...
    style_name = qt_font.styleName().strip() if (hasattr(qt_font, 'styleName') and qt_font.styleName()) else ''
    weight = int(qt_font.weight()) if hasattr(qt_font, 'weight') else int(QFont.Bold) if qt_font.bold() else int(QFont.Normal)
...
class FontWidget(QFontDialog):
    ...
    def _resolvedStyleText(self, font):
        ...
        if not font.bold():
            if not font.italic():
                for style, lowered_style in lowered:
                    if 'bold' not in lowered_style and 'italic' not in lowered_style and 'oblique' not in lowered_style:
                        return style
...
class QuranFontOptions(QWidget):
    ...
    def _fontGroup(self, title, size_key, spacing_key):
        ...
        return (size_combo, line_combo, group)
...
class OptionsWindow(CustomDialog):
    ...
    def _settingsSnapshot--- This code section failed: ---

 L. 724         0  LOAD_GLOBAL              Settings
                2  LOAD_METHOD              getValue
                4  LOAD_STR                 'theme_mode'
                6  CALL_METHOD_1         1  '1 positional argument'
                8  POP_TOP          

 L. 725        10  LOAD_CLOSURE             'self'
               12  BUILD_TUPLE_1         1 
               14  LOAD_DICTCOMP            '<code_object <dictcomp>>'
               16  LOAD_STR                 'OptionsWindow._settingsSnapshot.<locals>.<dictcomp>'
               18  MAKE_FUNCTION_8          'closure'

 L. 727        20  LOAD_DEREF               'self'
               22  LOAD_METHOD              _previewKeys
...
Parse error at or near `LOAD_DICTCOMP' instruction at offset 14
    ...
    def _changedPreviewKeys--- This code section failed: ---

 L. 731         0  LOAD_CLOSURE             'after_snapshot'
                2  LOAD_CLOSURE             'before_snapshot'
                4  LOAD_CLOSURE             'self'
                6  BUILD_TUPLE_3         3 
                8  LOAD_SETCOMP             '<code_object <setcomp>>'
               10  LOAD_STR                 'OptionsWindow._changedPreviewKeys.<locals>.<setcomp>'
               12  MAKE_FUNCTION_8          'closure'

 L. 732        14  LOAD_DEREF               'self'
               16  LOAD_METHOD              _previewKeys
...
class ThemeWidget(QWidget):
    ...
    def _appearanceState(theme_mode, use_modern_design, use_modern_icons):
        ...
        if effective_theme == Settings.THEME_DARK:
            return (
             effective_theme, None, 'dark')
        return (
         effective_theme, bool(use_modern_design), 'light' if use_modern_icons else 'old')
...
class SelectFolder(QGroupBox):
    ...
    def confirm(self, ok=None):
        ...
        if old_value != new_value:
            ...
            if ok:
                if is_writable and self.check_function:
