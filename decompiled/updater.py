            ...
            if status_code == 200:
                ...
                if self.major_online == self.major_ondisk:
                    if self.minor_ondisk == self.minor_online:
                        self.abandonBook()
                    elif 'minor_release_url' in response:
...
class Importer(QThread):
    ...
    def prepareProgress--- This code section failed: ---

 L. 457         0  LOAD_FAST                'read'
                2  LOAD_CONST               0
                4  COMPARE_OP               <
               ...
               12  LOAD_FAST                'self'
               14  LOAD_ATTR                books_total
               16  POP_JUMP_IF_TRUE     22  'to 22'
             18_0  COME_FROM            10  '10'
             18_1  COME_FROM             6  '6'

 L. 457        18  LOAD_CONST               None
               20  RETURN_VALUE     
             22_0  COME_FROM            16  '16'

 L. 458        22  LOAD_GLOBAL              Across
               24  LOAD_ATTR                main_window
               26  LOAD_ATTR                update_widget
               28  LOAD_ATTR                button_state
               30  LOAD_STR                 'prepare'
               32  COMPARE_OP               !=
               34  POP_JUMP_IF_FALSE    40  'to 40'

 L. 458        36  LOAD_CONST               None
               38  RETURN_VALUE     
             40_0  COME_FROM            34  '34'

 L. 459        40  LOAD_GLOBAL              min
               42  LOAD_CONST               99
               44  LOAD_GLOBAL              max
               46  LOAD_CONST               1
               48  LOAD_GLOBAL              int
               ...
               66  STORE_FAST               'percent'

 L. 460        68  LOAD_FAST                'percent'
               70  LOAD_FAST                'self'
               72  LOAD_ATTR                book_high
               74  COMPARE_OP               <=
               76  POP_JUMP_IF_FALSE    82  'to 82'

 L. 460        78  LOAD_CONST               None
               80  RETURN_VALUE     
             82_0  COME_FROM            76  '76'

 L. 461        82  LOAD_FAST                'percent'
               84  LOAD_FAST                'self'
               86  STORE_ATTR               book_high

 L. 462        88  LOAD_FAST                'self'
               90  LOAD_METHOD              prepareBar
               92  LOAD_FAST                'self'
               94  LOAD_ATTR                books_done
               96  LOAD_CONST               100
               98  BINARY_MULTIPLY  
              100  LOAD_FAST                'percent'
              102  BINARY_ADD       
              104  CALL_METHOD_1         1  '1 positional argument'
              106  POP_TOP          

Parse error at or near `CALL_METHOD_1' instruction at offset 104
    ...
    def process(self, item):
        ...
        if major_path:
            ...
            for path in [major_path, minor_path]:
                if path:
                    return isZipValid(path) or None
...
class PdfDownloader:
    ...
    def prepareBook(self):
        ...
        for pdf in self.files:
            ...
            if pdf['url'] not in valve:
                os.path.isfile(pdf['file']) or valve.add(pdf['url'])
    ...
    def getItem(self):
        while self.next_index < len(self.pending) and self.inflight < self.MAX_INFLIGHT:
            i = self.next_index
            self.next_index += 1
            self.launchItem(i)
    ...
    def itemFailed(self, epoch, i):
        ...
        if self.item_retries[i] <= self.RETRY_LIMIT:
            delay = stopped or min(self.RETRY_BASE_DELAY * 2 ** (self.item_retries[i] - 1), 60000)
